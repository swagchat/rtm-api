...
func TestPostRoom(t *testing.T) {
	...
	testTable := []testRecord{
		...
		},
		{
			...
			`,
			out:            `(?m)^{"error":{"message":"Room creation failed. \[PostRoom-002\] .*"}}$`,
			httpStatusCode: 500,
		},
		{
			testNo: 9,
			in: `
			`,
			out:            `(?m)^{"error":{"message":"Room creation request error. \[PostRoom-001\] .*"}}$`,
			httpStatusCode: 400,
		},
		{
			testNo: 10,
...
func TestGetRoom(t *testing.T) {
	...
	testTable := []testRecord{
		...
		},
		{
			...
			out:            `(?m)^{"error":{"message":"Get room information failed. \[GetRoom-001\] .*"}}$`,
			httpStatusCode: 500,
		},
	}
...
func TestPutRoom(t *testing.T) {
	...
	testTable := []testRecord{
		...
		},
		{
			...
			in: `
			`,
			out:            `(?m)^{"error":{"message":"Room update request error. \[PutRoom-001\] .*"}}$`,
			httpStatusCode: 400,
		},
		{
			testNo: 8,
